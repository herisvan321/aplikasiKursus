...
@section('content')
    ...
    <section class="content">
        ...
        <div class="card">
            ...
            <div class="card-body">
                @if (@count($data) > 0)
                    <form class="form-horizontal" method="POST" action="{{ route('kursus.views.update.text', [base64_encode($data->id_data_text)]) }}"
                        enctype="multipart/form-data">
                        @method("PUT")
                @else
                    <form class="form-horizontal" method="POST" action="{{ route('kursus.views.save.text', [$id]) }}"
                        enctype="multipart/form-data">
                @endif
                @csrf
                <div class="card-body">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="judul"
                                ...
                                value="{{ @count($data) > 0 ? $data->judul_data_text : '' }}">
                            @error('judul')
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <textarea name="content"
                                ...
                                rows="8"
                                placeholder="Enter Content">{{ @count($data) > 0 ? $data->content : '' }}</textarea>
                            @error('content')
                                <span class="error invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
