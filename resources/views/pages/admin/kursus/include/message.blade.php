...
<section class="content">
    <blockquote>
        ...
        <form action="{{ route('kursus.views.update.message', [base64_encode($data->id_data_message)]) }}" method="POST">
            @method("PUT")
        @else
        <form action="{{ route('kursus.views.save.message', [$id]) }}" method="POST">
        @endif
        @csrf
        @if(@count($data->id_data_message) > 0)
