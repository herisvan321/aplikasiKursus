    ...
    <tbody>
        @foreach ($data as $key => $dat)
            <tr>
                ...
                <td><a href="{{ route('mentor.kursus.menu.edit', [base64_encode($dat->id_sub_menu_kursus), $id]) }}"
                        class="btn btn-block btn-success btn-sm">Edit</a>
                    <a href="{{ route('mentor.kursus.views', [base64_encode($dat->id_sub_menu_kursus)]) }}"
                        class="btn btn-block btn-info btn-sm">View{{ $kondisi == 1 ? "($dat->count)" : "" }}</a>
                </td>
